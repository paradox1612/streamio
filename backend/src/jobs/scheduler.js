...
const { jobQueries, pool } = require('../db/queries');
...
async function withJobLock(jobName, worker) {
  ...
  if (!rows[0]?.acquired) {
    logger.warn(`[Job] Skipping ${jobName}; another instance already holds the lock`);
...
async function healthCheckJob() {
  return withJobLock('healthCheckJob', async () => {
...
async function tmdbSyncJob() {
  return withJobLock('tmdbSyncJob', async () => {
...
async function catalogRefreshJob() {
  return withJobLock('catalogRefreshJob', async () => {
...
async function epgRefreshJob() {
  return withJobLock('epgRefreshJob', async () => {
...
async function freeAccessExpiryJob() {
  return withJobLock('freeAccessExpiryJob', async () => {
    ...
    try {
      ...
      await jobQueries.finish(jobId, {
        status: 'success',
        metadata: getJobRunnerMetadata(result),
      });
...
async function freeAccessCatalogRefreshJob() {
  return withJobLock('freeAccessCatalogRefreshJob', async () => {
    ...
    try {
      ...
      await jobQueries.finish(jobId, {
        status: 'success',
        metadata: getJobRunnerMetadata({ refreshed }),
      });
