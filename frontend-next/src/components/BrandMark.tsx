...
export default function BrandMark({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-3">
      <div
        className={`relative flex items-center justify-center rounded-[18px] border border-white/10 bg-white/[0.04] shadow-[0_18px_45px_rgba(8,16,31,0.38)] ${
          compact ? 'h-10 w-10 sm:h-11 sm:w-11' : 'h-14 w-14'
        }`}
      >
      ...
      <div className="min-w-0">
        ...
        <h1 className={`${compact ? 'text-base sm:text-lg' : 'text-xl'} truncate font-bold text-white`}>
          StreamBridge
        </h1>
      </div>
    </div>
