...
export const Navbar = ({
  ...
  return (
    <motion.div className={cn('fixed inset-x-0 top-0 z-40 w-full px-3 pt-3 sm:px-5', className)}>
      {Array.isArray(children)
        ? children.map((child) =>
            child && typeof child === 'object' && 'props' in child
              ? { ...child, props: { ...child.props, visible } }
              : child
          )
        : children}
    </motion.div>
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div layoutId="hovered" className="absolute inset-0 h-full w-full rounded-full bg-white/[0.08]" />
          )}
...
export const MobileNavToggle = ({ isOpen }: { isOpen?: boolean }) => {
  return isOpen ? <X className="h-4 w-4 text-white" /> : <Menu className="h-4 w-4 text-white" />
}
...
export const NavbarButton = ({
  ...
  const variantStyles: Record<string, string> = {
    ...
    gradient:
      'bg-gradient-to-b from-brand-400 to-brand-600 text-white shadow-[0px_2px_0px_0px_rgba(255,255,255,0.24)_inset]',
  }
