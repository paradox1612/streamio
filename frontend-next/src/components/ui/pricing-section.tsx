...
function savingsLabel(monthly: number, yearly: number) {
...
function PricingCard({ plan, frequency }: { plan: Plan; frequency: Frequency }) {
  const savings = frequency === 'yearly' ? savingsLabel(plan.price.monthly, plan.price.yearly) : null
    ...
    >
      ...
      <div className="relative border-b border-white/[0.08] p-6">
        ...
        <div className="flex items-end gap-2">
          ...
          <span className="pb-1 text-sm text-slate-300/55">/{frequency === 'monthly' ? 'month' : 'year'}</span>
        </div>
        <div className="mt-4 flex min-h-6 items-center gap-2">
          {savings ? (
            ...
            <span className="text-xs uppercase tracking-[0.16em] text-slate-400/55">Flexible monthly billing</span>
          )}
        </div>
      </div>
  ...
  return (
    <div className={cn('w-full', className)}>
      <div className="mx-auto max-w-3xl text-center">
        ...
        {description && <p className="mt-4 text-base leading-7 text-slate-300/70">{description}</p>}
      </div>
