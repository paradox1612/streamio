...
export default function AdminUsersPage() {
  ...
  const load = async (term: string, isInitialLoad = false) => {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => { load(search, loading) }, 220)
  ...
  const filteredUsers = useMemo(
    () =>
      users.filter((user) => {
        const matchesStatus =
          statusFilter.length === 0 || statusFilter.includes(user.is_active ? 'active' : 'suspended')
        const matchesAccess =
          accessFilter.length === 0 || accessFilter.includes(user.free_access_status || 'inactive')
  ...
  const metrics = useMemo(() => {
    ...
    const freeAccessActive = filteredUsers.filter((u) => u.free_access_status === 'active').length
    const withProviders = filteredUsers.filter((u) => Number(u.provider_count || 0) > 0).length
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (user: any) => {
        ...
        return <Badge variant={variant as any} className="w-fit capitalize">{status}</Badge>
      },
    },
    {
      key: 'status',
    ...
    },
    {
      ...
      render: (user: any) => (
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="destructive" size="sm" className="rounded-xl" onClick={() => handleDelete(user)}>
            Delete
          </Button>
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="overflow-hidden">
          <CardContent className="p-6 sm:p-7">
            ...
            <h1 className="mt-3 text-3xl font-bold text-white sm:text-4xl">
              User access, provider usage, and free-access state in one table.
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300/65">
              Search remains backed by the API, while status and free-access filters run instantly client-side so
              operators can narrow down action lists without extra network churn.
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Healthy accounts" value={metrics.active} detail="Currently allowed to sign in and operate." icon={Shield} tone="border-emerald-400/20 bg-emerald-400/10 text-emerald-200" />
        <MetricCard label="Suspended" value={metrics.suspended} detail="Accounts currently blocked from access." icon={UserRoundX} tone="border-red-400/20 bg-red-500/10 text-red-200" />
        <MetricCard label="Free access active" value={metrics.freeAccessActive} detail="Users inside a current trial window." icon={Gift} tone="border-amber-400/20 bg-amber-400/10 text-amber-200" />
        <MetricCard label="With providers" value={metrics.withProviders} detail="Accounts that already configured catalog sources." icon={UserRoundCog} tone="border-sky-400/20 bg-sky-400/10 text-sky-200" />
      </div>
